...
async function exportJson() {
  try {
    ...
    if(d.ok) {
      toast('Отчет сохранен в ' + d.filename, 1);
    }
  } catch(e) {
    toast('Ошибка экспорта', 0);
  }
}
...
async function scan(){
 ...
 <button class="scan-btn" onclick="exportJson()" style="margin-top: 10px; background: #2a2a2a;">
...
def api_export():
    """Экспорт последнего отчета в JSON файл"""
    a = SecurityAuditor()
    findings = a.run_all()
    ...
    try:
        ...
        return jsonify({"ok": True, "filename": filename, "count": len(findings)})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500
...
if __name__ == '__main__':
    ...
    if not is_root: 
        print("[!] ВНИМАНИЕ: Скрипт запущен без sudo. Многие проверки будут недоступны.")
